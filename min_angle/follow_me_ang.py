...
min_dist= 11.3 # cm from direct measurement- 11.3 cm from calculations
...
while not rospy.is_shutdown():
    ...
    if driving_forward == 1: # If driving forward
        ...
        speed_L= -1
        speed_R=  1
        if dist > min_dist:  #The obstacle is the 'empty' space
        # if dist < min_dist:
            driving_forward = -1
    else:
        # Move backward
        speed_L=  1
        speed_R= -1
        if dist <= min_dist: # Go ahead while in the inner of the table
        #if dist >= min_dist:
            driving_forward = 1
